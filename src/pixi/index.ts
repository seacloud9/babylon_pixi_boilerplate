...
export const pixiCreateRendererAndStage = (
  engine: any,
  canvas: any
): { pixiRenderer: PIXI.WebGLRenderer; stage: PIXI.Container } => {
  // pixi.js rendering
  ...
  const addLineStyles = () => {
    ...
    app.ticker.add(() => {
      for (let i = 0; i < animatedLines.length; i++) {
        ...
        animatedLines[i].graphic.lineStyle(
          animatedLines[i].graphicStyle.width,
          animatedLines[i].graphicStyle.color,
          animatedLines[i].graphicStyle.alpha
        );

        if (animatedLines[i].moveY < window.innerHeight * -2) {
          stage.removeChild(animatedLines[i].graphic);
          animatedLines[i] = getLineObj();
        ...
        animatedLines[i].graphic.moveTo(
          animatedLines[i].moveX,
          animatedLines[i].moveY + count * -2
        );
        animatedLines[i].graphic.lineTo(
          animatedLines[i].lineX,
          animatedLines[i].lineY + count * -2
        );
      }
    });
  };
